...
def create_pico_ui():
    ...
    def pico_ui_page(request: Request):
        ...
        if not is_authenticated(request):
            ...
            with frame('Contrôle Picoscope 5444D', request):
                ...
                def gen_signal():
                    if sig_generator.enabled == True:
                        if pico.type_signal == "None":
                            pico.remove_signal()
                        else:
                            pico.set_signal() # Tous les paramètres sont déjà stockés dans l'objet pico, pas besoin de les passer
                    else:
                        pico.type_signal == "None"

                # Mise à jour de la fréquence du signal
                def set_signal_frequency():
                    if sig_generator.enabled:
                ...
                def set_acquisition_state():
                    if acquisition_state_button.enabled:
                ...
                def update_ranges():
                    for channel in ['A', 'B', 'C', 'D']:
                        if channel_range_toggles[channel].enabled:
                ...
                delay = settings['plot_update_timer'] # On récupère le paramètre
                data_update = ui.timer(delay, do_data_update, active=False)
